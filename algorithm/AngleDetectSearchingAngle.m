...
if global_max_amp / global_mean_amp < 2.0     % 确定当前幅度值之差是否比较能明确探测到目标 
    return % 幅度太小了 肯定探测不到
end
    ...
    for ii = 1:size(cfar_detect_info, 1)
        ...
        % 寻找响应最大的距离
        r_index = range_search(angle_map, r_index, 10);
        ...
        valid_index = find(a_values > max_amp * 0.9 ...
            & a_values > global_mean_amp * 1.2 ...
            & mean_amp > global_mean_amp * 0.8);
%         valid_index = find(a_values > max_amp * 0.8);
...
function range_searched_index = range_search(az_map, range_index, offset)
    ...
    for ii = 1:length(ranges)
        [val, index] = max(az_map(ranges(ii), :)); % 找到最大值
