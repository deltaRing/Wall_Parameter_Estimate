...
% 输出2：聚类数量   cluster_num
function [data_cluster, cluster_num] = DBSCAN(data, eps, minpoints)
    % 提供默认值
    if nargin == 1
        eps = 0.25;      % 最大半径是0.25
        minpoints = 100; % 最小点数是100个点
    end
    % 数据是否已经被聚类
    ...
    if 0
        ...
        title("DBSCAN聚类结果 EPS=0.25, point=100")
        xlabel("距离 (meter)")
        ylabel("速度 (m/s)")
        zlabel("角度 (rad)")
        legend
