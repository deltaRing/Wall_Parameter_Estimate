...
% 感兴趣的角度单元：angle_unit 
% 幅度信息：amp （去除二次多径反射）
function [detect_units, amps] = rd_select_unit(rd_map,ra_map,ra_map_el, ...
            range_axis,doppler_axis,angle_axis, ...
            drone_height, disable_velo)
    % 1.6米一下的速度
    if nargin == 6, disable_velo = 20; drone_height = 1.4; end 
    % 无人机高度所对应的距离单元求解
    drone_height_unit = sum(drone_height > range_axis); % 防止高度噪声带来对检测结果的影响
    % 找到中心速度
    ...
    % 屏蔽相关的距离单元
    detect_map_rd(drone_height_unit - 10:drone_height_unit, :) = 0;
    detect_map_rd(drone_height_unit:drone_height_unit + 15, :) = 0;
    % 滤除相关的检测结果
    ...
    detect_result_rd(find(abs(detect_result_rd(:, 1) - drone_height_unit) < 10),:) = [];  % 距离滤除
    % 根据以上的检测结果，遍历RA谱图
    detect_result_ra    = ra_select_unit(ra_map, detect_result_rd);
    detect_result_ra_el = ra_select_unit(ra_map_el, detect_result_rd);
    % 组合数据 将其重映射到具体的距离轴
